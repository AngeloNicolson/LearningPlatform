/**
 * @file BiblicalHistoryPage.tsx
 * @author Angelo Nicolson
 * @brief Biblical History resources page with time period filtering
 * @description Biblical History learning page providing access to historical resources organized by time periods (Creation to Patriarchs, Exodus, Kings, Exile, etc.).
 ...
 * focused on biblical timeline, historical events, and cultural contexts.
 */
...
interface BiblicalHistoryPageProps {
  onBack?: () => void;
}

export const BiblicalHistoryPage: React.FC<BiblicalHistoryPageProps> = ({ onBack }) => {
  const navigation = useNavigation();
  const [selectedTopic, setSelectedTopic] = useState<string>(navigation.currentState.biblicalHistoryTab || 'all');
  ...
  useEffect(() => {
    const fetchSubjectData = async () => {
      try {
        ...
        const subjectResponse = await authFetch(`${import.meta.env.VITE_API_URL || 'https://localhost:3777/api'}/subjects/biblical-history`, {
          credentials: 'include'
        });
        ...
        const levelsResponse = await authFetch(`${import.meta.env.VITE_API_URL || 'https://localhost:3777/api'}/subjects/biblical-history/levels`, {
          credentials: 'include'
        });
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const response = await authFetch(`${import.meta.env.VITE_API_URL || 'https://localhost:3777/api'}/resources/biblical-history/topics`, {
          credentials: 'include'
        });

        if (response.ok) {
          const data = await response.json();
          console.log('[BiblicalHistoryPage] Fetched topics:', data);
          setApiTopics(data);
        } else {
          console.error('[BiblicalHistoryPage] Failed to fetch topics, status:', response.status);
        }
      } catch (error) {
        console.error('[BiblicalHistoryPage] Error fetching topics:', error);
      }
    };
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        const response = await authFetch(`${import.meta.env.VITE_API_URL || 'https://localhost:3777/api'}/resources/biblical-history?${params.toString()}`, {
          credentials: 'include'
        });
  ...
  useEffect(() => {
    if (navigation.currentState.biblicalHistoryTab) {
      setSelectedTopic(navigation.currentState.biblicalHistoryTab);
    }
  }, [navigation.currentState.biblicalHistoryTab]);

  // Update navigation when topic changes
  const handleTopicChange = (topic: string) => {
    setSelectedTopic(topic);
    navigation.navigate({ biblicalHistoryTab: topic });
  };
  ...
  const comingSoonFeatures = [
    { icon: '🗺️', name: 'Interactive Timeline' },
    { icon: '🏛️', name: 'Archaeology' },
    { icon: '📜', name: 'Primary Sources' },
    { icon: '🌍', name: 'Cultural Context' }
  ];
  ...
  return (
    <ResourcePageLayout
      ...
      tagline="Explore the historical timeline and events of the Bible"
      topics={apiTopics}
      resources={apiResources}
      resourceTypes={resourceTypes}
      gradeFilters={gradeFilters}
      topicsPerPage={4}
      searchPlaceholder="Search biblical history resources..."
      historyStorageKey="biblicalHistoryResourcesHistory"
